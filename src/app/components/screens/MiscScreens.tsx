...
export const MenuScreen = () => (
  <div className="space-y-6">
    <div className="flex items-center justify-between">
      <div>
        <h1 className="text-3xl font-semibold text-primary mb-2">Menu Management</h1>
        <p className="text-muted-foreground">Manage dishes, combos, beverages, and wedding packages</p>
      </div>
    ...
    <div className="grid grid-cols-4 gap-6">
      {[
        { title: "Dishes", count: 48, color: "#c9a961" },
        { title: "Dish Combos", count: 12, color: "#1a1a2e" },
        { title: "Beverages", count: 24, color: "#2563eb" },
        { title: "Wedding Packages", count: 8, color: "#16a34a" },
      ].map((item) => (
        <div key={item.title} className="bg-card rounded-[20px] p-6 border border-border shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div className="w-12 h-12 rounded-xl flex items-center justify-center" style={{ backgroundColor: item.color + "1a" }}>
              <BarChart3 className="w-6 h-6" style={{ color: item.color }} />
            </div>
          </div>
          <p className="text-sm text-muted-foreground mb-1">{item.title}</p>
          <h3 className="text-2xl font-semibold text-primary">{item.count}</h3>
        </div>
      ))}
    </div>
    ...
    <div className="bg-card rounded-[20px] border border-border shadow-sm overflow-hidden">
      <div className="p-6 border-b border-border">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-semibold text-primary">Menu Items</h3>
          <div className="flex gap-2">
            {["All", "Dishes", "Combos", "Beverages", "Packages"].map((tab, i) => (
              <button key={tab} className={`px-4 py-2 rounded-xl text-sm ${i === 0 ? "bg-primary text-primary-foreground" : "border border-border hover:bg-secondary"}`}>{tab}</button>
            ))}
          </div>
        </div>
      </div>
      <div className="p-6">
        <div className="grid grid-cols-3 gap-4">
          {[
            { name: "Premium Wedding Package A", type: "Package", price: 25000000, description: "Includes premium dishes, beverages, and decoration" },
            { name: "Deluxe Combo Set", type: "Combo", price: 800000, description: "5 premium dishes + dessert + beverage" },
            { name: "Lobster with Butter Sauce", type: "Dish", price: 350000, description: "Fresh lobster served with garlic butter" },
            { name: "Imported Wine Selection", type: "Beverage", price: 1200000, description: "Premium red and white wine collection" },
            { name: "Standard Wedding Package B", type: "Package", price: 18000000, description: "Complete wedding service package" },
            { name: "Wagyu Beef Steak", type: "Dish", price: 450000, description: "Premium wagyu beef with truffle sauce" },
          ].map((item, idx) => (
            <div key={idx} className="bg-secondary rounded-xl p-4 border border-border hover:border-accent transition-all">
              ...
              <div className="flex items-center justify-between">
                <span className="text-xs px-2 py-1 bg-accent/20 text-accent rounded-full">{item.type}</span>
                <span className="text-sm font-semibold text-accent font-mono">{formatVND(item.price)}</span>
              </div>
              <div className="flex gap-2 mt-3">
                <button className="flex-1 py-2 text-xs border border-border rounded-lg hover:bg-card transition-colors">Edit</button>
                <button className="flex-1 py-2 text-xs border border-border rounded-lg hover:bg-card transition-colors">View</button>
              </div>
            </div>
          ))}
        </div>
